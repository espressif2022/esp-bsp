...
static esp_err_t esp_lcd_touch_gt911_read_data(esp_lcd_touch_handle_t tp)
{
    ...
    if ((buf[0] & 0x80) == 0x00) {
        ...
        for (i = 0; i < touch_cnt; i++) {
            ...
            tp->data.coords[i].x = tp->data.coords[i].x * 480 / 320;
            tp->data.coords[i].y = tp->data.coords[i].y * 360 / 240;
        }

        portEXIT_CRITICAL(&tp->data.lock);
        // ESP_LOGE(TAG, "touch_cnt:%d, [%d,%d]!", touch_cnt, tp->data.coords[0].x, tp->data.coords[0].y);
    }
