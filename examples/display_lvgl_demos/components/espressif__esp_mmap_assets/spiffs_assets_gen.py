...
def pack_assets(config: PackModelsConfig):
    ...
    with open(file_path, 'w') as output_header:
        ...
        output_header.write(f'#define MMAP_{asset_name.upper()}_OFFSET          0x{start_offset:04X}\n\n')
...
if __name__ == '__main__':
    ...
    if os.path.exists(append_bin_path):
        os.remove(append_bin_path)
    ...
    if total_size > partition_size:
        print(f'Recommended assets partition size: {GREEN}{recommended_size}K{RESET}')
        print(f'{RED}Error:Combined binary size exceeds partition size.{RESET}')
        sys.exit(1)
    ...
    print(f'Combined binary created: {os.path.basename(combined_bin_path)}')

    shutil.move(combined_bin_path, app_bin_path)
    print(f'Overwritten {os.path.basename(app_bin_path)} with {os.path.basename(combined_bin_path)}')
