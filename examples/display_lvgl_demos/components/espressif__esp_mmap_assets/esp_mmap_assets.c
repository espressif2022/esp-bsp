...
esp_err_t mmap_assets_new(const mmap_assets_config_t *config, mmap_assets_handle_t *ret_item)
{
    ...
    uint32_t offset = config->start_offset;
    uint32_t size = partition->size - offset;
    ...
    if (config->flags.full_check) {
        ESP_GOTO_ON_FALSE(calculated_checksum == stored_chksum, ESP_ERR_INVALID_CRC, err, TAG, \
                          "bad full checksum, cal:0x%x, read:0x%x", (unsigned int)calculated_checksum, (unsigned int)stored_chksum);
    }
