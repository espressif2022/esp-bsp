                          ...
                          uint32_t start, uint32_t * glyph_offset, uint32_t loca_count, font_header_bin_t * header)
{
    ...
    ESP_LOGI(TAG, "glyph_dsc, loca_count:%d, size:%d", loca_count, loca_count * sizeof(lv_font_fmt_txt_glyph_dsc_t));

    font_dsc->glyph_dsc = glyph_dsc;

    int cur_bmp_size = 0;

    ESP_LOGI(TAG, "header, xy_bits:%d, wh_bits:%d, advance_width_bits:%d", header->xy_bits, header->wh_bits, header->advance_width_bits);
    ...
    ESP_LOGI(TAG, "glyph_bmp, cur_bmp_size:%d, malloc:%d", cur_bmp_size, sizeof(uint8_t) * cur_bmp_size);
