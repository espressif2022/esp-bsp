...
static int32_t load_cmaps(lv_fs_file_t *fp, lv_font_fmt_txt_dsc_t *font_dsc, uint32_t cmaps_start)
{
    ...
    ESP_LOGI(TAG, "cmaps_subtables_count: 0x%x, size:%02X", cmaps_subtables_count, cmaps_subtables_count * sizeof(lv_font_fmt_txt_cmap_t));
...
static uint8_t * load_glyph_bitmap(void *font_dsc, uint32_t glyph_id, uint8_t *buffer, uint32_t offset) {
    
    lv_font_fmt_txt_dsc_t *dsc = (lv_font_fmt_txt_dsc_t *)font_dsc;
    uint8_t *glyph_bmp = dsc->glyph_bitmap;
    ESP_LOGE(TAG, "load_glyph_bitmap, font_dsc:%p, glyph_id:%d, offset:%d, %02X", font_dsc, glyph_id, offset, dsc->glyph_start + dsc->glyph_offset[offset]);

    lv_fs_res_t res = lv_fs_seek(dsc->fp, dsc->glyph_start + dsc->glyph_offset[offset], LV_FS_SEEK_SET);
        if (res != LV_FS_RES_OK) {
            return NULL;
        }
        bit_iterator_t bit_it = init_bit_iterator(dsc->fp);

        int nbits = dsc->nbits;//== 26
        ...
        // int next_offset = (offset < loca_count - 1) ? dsc->glyph_offset[offset + 1] : (uint32_t)glyph_length;
        int next_offset = dsc->glyph_offset[offset + 1];
        int bmp_size = next_offset - dsc->glyph_offset[offset] - nbits / 8;

        // glyph_bmp = lv_mem_alloc(bmp_size);
        // assert(glyph_bmp != NULL);
        // ESP_LOGI(TAG, "malloc bmp_size:%d", bmp_size);

        if (nbits % 8 == 0) { /*Fast path*/
            if (lv_fs_read(dsc->fp, &glyph_bmp[0], bmp_size, NULL) != LV_FS_RES_OK) {
                lv_mem_free(glyph_bmp);
                return NULL;
            }
        } else {
...
static bool lvgl_load_font(lv_fs_file_t *fp, lv_font_t *font)
{
    ...
    ESP_LOGI(TAG, "glyph_start: 0x%x, loca_count:%d, font_dsc:%p, glyph_offset:%p, fp:%p", glyph_start, loca_count, font_dsc, glyph_offset, font_dsc->fp);
